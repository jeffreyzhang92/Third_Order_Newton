...
import math
from Newton_Functions import init_func
from Newton_Functions import init_params

#%%

[fX, fx, dx, d2x, d3x] = init_func('Himmelblau')
[XMIN, XMAX, YMIN, YMAX, x_min] = init_params('Himmelblau')
...
for xi in range(RES2):
    for yi in range(RES2):
        ...
        while k < MAX_ITS and LA.norm(dx(x_curr)) > .001:
...
for xi in range(RES2):
    for yi in range(RES2):
        for i in range(num_mins):
            if LA.norm(x_min[i,:]-fractal_2[yi,xi,0:2]) < .001:
                colors_2[yi,xi] = num_mins - i
                break
            colors_2[yi,xi] = 0
...
cp = ax.contourf(X, Y, colors_2, cmap = plt.get_cmap('viridis'))
...
fig,ax=plt.subplots(1,1)
cp = ax.contourf(X, Y, colors_3, cmap = plt.get_cmap('viridis'))
ax.contour(X,Y,np.log(Z),colors='red')
ax.set_xlabel('x')
ax.set_ylabel('y')
plt.show()
