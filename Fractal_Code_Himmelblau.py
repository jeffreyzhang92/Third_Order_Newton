...
RES3 = 101
MAX_ITS = 200
x_min = np.reshape(np.array([[3,2]]),(2,1))
...
for yi in range(RES2):
    for xi in range(RES2):
        if LA.norm(x_min.T-fractal_2[yi,xi,0:2]) < .001:
            colors_2[yi,xi] = 5
        elif LA.norm(np.array([-2.805118,3.131312])-fractal_2[yi,xi,0:2]) < .001:
            colors_2[yi,xi] = 4
        elif LA.norm(np.array([-3.779310,-3.283186])-fractal_2[yi,xi,0:2]) < .001:
            colors_2[yi,xi] = 3
        elif LA.norm(np.array([3.584428,-1.848126])-fractal_2[yi,xi,0:2]) < .001:
            colors_2[yi,xi] = 2
        else:
            colors_2[yi,xi] = 1
...
for xi in range(RES3):
    for yi in range(RES3):
        if LA.norm(x_min.T-fractal_3[yi,xi,0:2]) < .001:
            colors_3[yi,xi] = 5
        elif LA.norm(np.array([-2.805118,3.131312])-fractal_3[yi,xi,0:2]) < .001:
            colors_3[yi,xi] = 4
        elif LA.norm(np.array([-3.779310,-3.283186])-fractal_3[yi,xi,0:2]) < .001:
            colors_3[yi,xi] = 3
        elif LA.norm(np.array([3.584428,-1.848126])-fractal_3[yi,xi,0:2]) < .001:
            colors_3[yi,xi] = 2
        else:
            colors_3[yi,xi] = 1
